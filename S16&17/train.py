...
import torch.optim as optim  # Add this if not already imported
...
def train_model(config):
    ...
    lr_max = config.get("lr_max", 1e-3)  # Maximum learning rate, can adjust based on your needs
    lr_min = config.get("lr_min", 1e-5)  # Minimum learning rate, can adjust based on your needs
    epochs = config["num_epochs"]
    steps_per_epoch = len(train_dataloader)
    max_at_epoch = config.get("max_at_epoch", epochs // 2)
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            ...
            # Update scheduler at each batch step
            scheduler.step()
